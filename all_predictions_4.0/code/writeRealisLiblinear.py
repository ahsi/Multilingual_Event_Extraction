...
def processSentence(curSentence, output, easyOutput, roleIndexDict, featureIndexDict, sentenceIndex, testSet=False):
    ...
    for triggerIndex in range(len(curWords)):
        ...
        if triggerLabel != EMPTY_TRIGGER:
            for entityIndex in range(len(curEnts)):
                processEntity(curSentence, entityIndex, triggerIndex, triggerLabel, output, easyOutput, roleIndexDict, featureIndexDict, sentenceIndex, testSet)

def processEntity(curSentence, entityIndex, triggerIndex, triggerLabel, output, easyOutput, roleIndexDict, featureIndexDict, sentenceIndex, testSet=False):
    curWords = curSentence.words
    ...
    # FOR REALIS ONLY
    if goldArgRole == "NONE" and not testSet:
        return

    argID = roleIndexDict[goldArgRole]
    output.write(str(argID))
    easyOutput.write("sent_" + str(sentenceIndex) + "\tPhrase:\t" + curEntity.text + "\tCorefStr:\t" + curEntity.corefStr + "\tRole:\t" + goldArgRole + "\tTrigger:\t" + triggerWord + "\tEventType:\t" + triggerLabel + "\tEntityType:\t" + curEntity.entType + "." + curEntity.subtype + "\tDOCID:\t" + curSentence.docID + "\tSTART:\t" + str(curEntity.start)  + "\tEND:\t" + str(curEntity.end) + "\tSent_START:\t" + str(curSentence.startOffset) + "\tSent_END:\t" + str(curSentence.endOffset))
    ...
    for feature in features:
        if testSet and feature not in featureIndexDict and not feature.startswith("WORD2VEC"):
            continue
...
def main():
    ...
    ### Training
    if trainMode:
        possibleLabels, possibleArgs = scanInput(textFile, textFile + ".parsing", realisMode = True)
        ...
        while True:
            if count == 0:
                sentence, valid, temp = readInput(input, parsingInput, count=triggerCount, entityOut=open(outputFile + ".entityCoref", 'w'), realisMode = True)
                triggerCount = temp
            else:
                sentence, valid, temp = readInput(input, parsingInput, count=triggerCount, entityOut=open(outputFile + ".entityCoref", 'a'), realisMode = True)
                triggerCount = temp
    ...
    ### Testing
        ...
        while True:
            # debug -- if the test data doesn't have labels, set realisMode off (so we don't get a blank test file)
            if triggerFile == "NONE":
                if count == 0:
                    sentence, valid, temp = readInput(input, parsingInput, entityOut=open(outputFile + ".entityCoref", "w"), realisMode = True)
                else:
                    sentence, valid, temp = readInput(input, parsingInput, entityOut=open(outputFile + ".entityCoref", "a"), realisMode = True)
            else:
                if count == 0:
                    sentence, valid, temp = readInput(input, parsingInput, testTriggers, count=triggerCount, entityOut=open(outputFile + ".entityCoref", "w"), realisMode = True)
                    triggerCount = temp
                else:
                    sentence, valid, temp = readInput(input, parsingInput, testTriggers, count=triggerCount, entityOut=open(outputFile + ".entityCoref", "a"), realisMode = True)
                    triggerCount = temp
            ...
            if sentence != None:
                processSentence(sentence, output, easyOutput, roleIndexDict, featureIndexDict, count, True)
